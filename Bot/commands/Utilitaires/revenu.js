...
module.exports.run = async (client, message, args) => {
    ...
    const groupe = args[0]
    if (!guild.groups.includes(groupe)) return message.reply('Merci d\'entrer un groupe valide ! Regarde tous les groupes possibles avec la commande `groups`')
    ...
    await client.updateAbsences(message.guild, message.author.id, groupe, 0)
    return message.reply('Ton retour a été enregistré avec succès ! Bon retour parmis nous ^^')
}
module.exports.help = MESSAGES.Commandes.Utilitaires.REVENU;
