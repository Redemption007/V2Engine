...
module.exports.run = async (client, message, _args, settings) => {
    ...
    const erreur = {embed: {title: 'Oups !', color: 'RED', description: 'La commande à été annulée car vous avez mis trop de temps à répondre.'}}
    let pseudo = ''
    let profil = ''
    let choix = ''
    ...
    if (tournoi.Compo > 1 && !tournoi.Random) {
        ...
        try {
            await msg.edit({embed: {title: 'Inscription : Etape 1', color: 'BLUE', description: 'Merci de renseigner votre pseudo de jeu :', footer: {text: 'Vous avez 1 minute maximum pour répondre à cette question.'}}})
            ...
            await message.author.send({embed: {color: 'BLUE', title: 'Voulez-vous :', description: desc}})
                .then(mesg => msg = mesg)
            ...
            switch (choix) {
            case '👥':
                await message.author.send({embed: {title: 'Inscription : Etape 2', description: 'Merci de renseigner votre code d\'équipe :', footer: {text: 'Vous avez 1 minute maximum pour répondre à cette question.'}}})
                await msg.channel.awaitMessages(filterNumber, {max: 1, idle: 30000, errors: 'time'})
                    .then(async collected => {
                        ...
                        if (team) {
                            ...
                            return message.author.send({embed: {title: 'Inscription réussie !', color: 'GREEN', description: "Votre équipe a été notifiée de votre arrivée :partying_face:", timestamp: Date.now()}})
                        }
                ...
                break
            case '🔎':
                await message.author.send({embed: {title: 'Inscription : Etape 2', description: "Merci d'écrire une courte description de votre profil :", footer: {text: "Vous avez 2 minute pour répondre à cette question."}, timestamp: Date.now()}})
        ...
        return msg.edit(`<@${message.author.id}>`, {embed: {title: 'Inscription finie !', description: 'Vous avez été inscrit avec succès à ce tournoi !', timestamp: Date.now(), footer: footer}})
    }
}
