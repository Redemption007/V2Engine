...
module.exports.run = async (client, message, _args, settings) => {
    ...
    try {
        ...
        await message.channel.awaitMessages({filter: filterDate, max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
        ...
        await message.channel.awaitMessages({filter: filterDuree, max: 1, idle: 30000, errors: 'time'})
            .then(coll => {
        ...
        await message.channel.awaitMessages({filter: filterMsg, max: 1, idle: 120000, errors: 'time'})
            .then(coll => {
                if (coll.first().content.toLowerCase() === 'cancel') throw new Error(canceled)
        ...
        await message.channel.awaitMessages({filter: filterNumber2a60, max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
                if (coll.first().content.toLowerCase() === 'cancel') throw new Error(canceled)
        ...
        if (room>2) {
            ...
            await message.channel.awaitMessages({filter: filterNumber1a10, max: 1, idle: 30000, errors: 'time'})
                .then(async coll => {
        ...
        await message.channel.awaitMessages({filter: filterNumber1a30, max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
