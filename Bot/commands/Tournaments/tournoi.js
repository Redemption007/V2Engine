...
module.exports.run = async (client, message, _args, settings) => {
    ...
    await message.channel.send(warning, {embed: {color: 'DARK_GREEN', title: 'La commande a été validée !', description: 'Quel sera le nom du tournoi ?'}})
        .then(mesg => msg = mesg)
        .catch(() => {
    ...
    try {
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Nom du tournoi validé !', description: `Le nom du tournoi sera ${nomdutournoi}`, fields: [{name: next, value: 'Quel est le prix à remporter du tournoi ?'}]}})
        await message.channel.awaitMessages(filterMsg, {max: 1, idle: 30000, errors: 'time'})
            .then(coll => {
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Prix validé !', description: `Le prix de votre tournoi sera : ${CP}`, fields: [{name: next, value: 'Quand le tournoi se déroulera-t-il ?\n__Format :__ `Le Jour/Mois`\n__Exemples :__ `Le 09/08` ou `le 01 01`'}]}})
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Date validée !', description: 'Quelle sera l\'heure du début du tournoi ?\n__Format :__ `Heures:Minutes`\n__Exemples :__ `A 09h08` ou `à 01 01`'}})
        await message.channel.awaitMessages(filterDate, {max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Durée validée !', description: `La durée de votre tournoi est de ${ms(duration, true)}.`, fields: [{name: next, value: `Quand commenceront les inscriptions ? (Avant le ${date.toLocaleTimeString('fr-FR', options)})\n__Format :__ \`Le Jour/Mois\`\n__Exemples :__ \`Le 09/08\` ou \`le 01/01\``}]}})
        await message.channel.awaitMessages(filterDate, {max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Date validée !', description: 'Quelle sera l\'heure du début des inscriptions ?\n__Format :__ `Heures:Minutes`\n__Exemples :__ `A 09h08` ou `à 01 01`'}})
        await message.channel.awaitMessages(filterDate, {max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
                let debutinsc = await coll.first().content.toLowerCase()
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Date des inscriptions validée !', description: `Les inscriptions commenceront le ${datedebutinsc.toLocaleTimeString('fr-FR', options)}`, fields: [{name: next, value: 'Combien de temps avant le début du tournoi les inscriptions s\'arrêteront-elles ?\n__Format :__ `nb[j/h/m]`\n__Exemples :__ `1h` ou `37 minutes` ou `90min`'}]}})
        await message.channel.awaitMessages(filterMsg, {max: 1, idle: 30000, errors: 'time'})
            .then(coll => {
                if (coll.first().content.toLowerCase() === 'cancel') throw new Error(canceled)
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', footer: {text: 'Vous avez 2 minutes pour répondre à cette question'}, title: 'Date de fin des inscriptions validée !', description: `Les inscriptions finiront ${ms(fin, true)} avant le début du tournoi.`, fields: [{name: next, value: "Quel sera le déroulement du tournoi ? (maps, matches, etc)"}]}})
        ...
        await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Nombre maximal d\'équipes validé !', description: `Il y aura un maximum de ${nbteam} équipes dans ce tournoi.`, fields: [{name: next, value: `Combien d'équipes s'affronteront dans un lobby ? (Donner un nombre entre 2 et ${nbteam})`}]}})
        await message.channel.awaitMessages(filterNumber2a60, {max: 1, idle: 30000, errors: 'time'})
            .then(async coll => {
        ...
        if (room>2) {
            await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Nombre d\'équipes par lobby enregistré !', description: `${room} équipes s'affronteront dans chaque room`, fields: [{name: next, value: `Combien il y aura-t-il d'équipes gagnantes par lobby (finale exceptée) ? (Donner un nombre entre 1 et ${Math.min(10, room-1)})`}]}})
            ...
            await await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Nombre d\'équipes gagnantes par lobby enregistré !', description: `Il y aura ${winners} gagnants par lobby, finale exceptée.`, fields: [{name: next, value: 'Combien il y aura-t-il de personnes par équipes ? (Donner un nombre entre 1 et 30)'}]}})
        } else {
            await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Nombre d\'équipes par lobby enregistré !', description: `${room} équipes s'affronteront dans chaque room (1 vainqueur par room)`, fields: [{name: next, value: 'Combien il y aura-t-il de personnes par équipes ? (Donner un nombre entre 1 et 30)'}]}})
        }
        ...
        if (compo > 1) {
            await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Composition des équipes validée !', description: `Il y aura ${compo} joueurs par équipe dans ce tournoi.`, fields: [{name: next, value: 'Le tournoi sera-t-il avec des équipes faites au hasard ?'}]}})
            await msg.react('✅')
            await msg.react('❌')
            ...
            await msg.edit(warning, {embed: {color: 'DARK_GREEN', title: 'Composition des équipes validée !', description: `Il y aura ${composition}`, fields: [{name: next, value: 'Voulez-vous autoriser les équipes incomplètes ?'}]}})
            await msg.react('✅')
            await msg.react('❌')
            await msg.awaitReactions(filterReaction, {max: 1, idle: 30000, errors: 'time'})
                .then(async coll => {
        ...
        await msg.awaitReactions(filterReaction, {max: 1, idle: 60000, errors: 'time'})
            .then(async coll => {
                ...
                switch (emoji) {
                case '✅':
                    await embed.run(client, message, "GREEN;; La commande a été validée !;; Création de la catégorie du Tournoi...")
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: 'La commande a été validée !', description: 'Création du channel d\'informations...'}})
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: 'La commande a été validée !', description: 'Création du channel d\'inscriptions...'}})
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: 'La commande a été validée !', description: 'Création du channel du staff...'}})
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: 'La commande a été validée !', description: 'Envoi des informations...'}})
                    await embed.run(client, StaffChannel, "DARK_GREEN;; Voici la liste des commandes à connaître :;; - `forceunregister` : obliger un membre à quitter un tournoi,\n- `list` : obtenir la liste des inscrits,\n- `check` : vérification des résultats,\n- `bantournoi` : bannissement d'un membre aux tournois,\n- `unbantournoi` : révocation du bannissement d'un membre aux tournois,\n- `deletetournoi` : suppression du tournoi.")
                        .then(mesg => mesg.pin())
                    await InfosChannel.send(`@everyone`, {embed: {
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: 'La commande a été validée !', description: 'Création du tournoi...'}})
                    ...
                    await validation.edit({embed: {color: 'GREEN', title: "Tournoi créé avec succès !"}})
