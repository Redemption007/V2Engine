...
module.exports.run = async (client, message) => {
    ...
    if (tournoi.Inscrits.length === 0) return message.channel.send({embed: {title: 'Ah...', footer: {text: "Réessayez dans quelques minutes !"}, description: 'Il n\'y a encore aucun inscrit dans ce tournoi...', color: "ORANGE"}})
    if (tournoi.Compo === 1 || tournoi.Random) {
        ...
        return message.channel.send({embed: {title: `Liste des participants au tournoi ${tournoi.NomduTournoi} :`, color: 'BLUE', fields: [{name: 'Participants :', value: inscrits, inline: true}, {name: 'Peudos de jeu :', value: pseudos, inline: true}]}, split: true})
    }
