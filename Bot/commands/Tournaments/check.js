...
module.exports.run = async (client, message) => {
    ...
    if (tournoi.NextRound[0] === "Finale") {
        ...
        const msgclassement = await message.reply({embed: {description: 'Merci de réagir avec le chiffre donnant le nombre d\'équipes qui seront affichées dans le classement.', color: 'DARK_GREEN', footer: {text: 'Vous avez 30 secondes pour réagir, après quoi la commande sera annulée.', icon_url: message.author.avatarURL()}}})
        ...
        try {
            for (let i=0; i<classement; i++) {
                msgclassement.edit({embed: {description: `Quelle est l'ID de l'équipe qui remporte ce tournoi ? (${i+1}${i<1?'ère':'ème'} place)`}})
        ...
        InfosChannel.send(`<@&${tournoi.RoleTournoi}>`, {embed: {color: 'BLUE', title: 'Classement du tournoi :', fields: [{name: 'Places :', value: desc, inline: true}, {name: 'Pseudos :', value: field, inline: true}], footer: {text: 'Les gagnants sont invités à DM les organisateurs du tournoi.'}}})
    ...
    await message.reply({embed: {description: `Donnez les ID d'${tournoi.Compo<2? 'utilisateurs' : 'équipes'} que vous voulez déclarer vainqueurs.\nSchéma : \`<id1> <id2> <id3> <etc>\``, color: 'PURPLE'}})
    await message.channel.awaitMessages(filterMessages, {max: 1, time: 30000})
    ...
    const mesg = await message.reply({embed: {color: 'ORANGE', title: `Confirmez vous déclarer gagnant${tournoi.Compo>2?'e':''}s ces ${tournoi.Compo<2? 'utilisateurs':'équipes'} ?`, fields: [{name: 'IDs :', value: ids, inline: true}, {name: 'Lobbys concernés :', value: lobbys, inline: true}], footer: {text: 'Vous avez 30 secondes pour confirmer', icon_url: message.author.avatarURL()}}})
    mesg.react('✅')
    mesg.react('❌')
    ...
    for (let i = 0; i < teams.length; i++) {
        lobb[i].send({embed: {title: 'Résultats :', color: 'GREEN', description: `${tournoi.Compo<2? `<@${teams[i].members[0].userid}>` : `L''équipe ${teams[i].id}`} a été désigné${tournoi.Compo>2?'e':''} vainqueur du lobby${tournoi.Gagnants<2? '. Merci de votre participation.' : '. Merci d\'attendre la parution des résultats complets.'}`}})
    }
    ...
    await mesg.edit({embed: {color: 'ORANGE', title: `Ces ${tournoi.Compo<2? 'utilisateurs':'équipes'} ont étés déclaré${tournoi.Compo>2?'e':''}s gagnant${tournoi.Compo>2?'e':''}s :`, fields: [{name: 'IDs :', value: ids, inline: true}, {name: 'Lobbys concernés :', value: lobbys, inline: true}], footer: {icon_url: message.author.avatarURL()}}})
    ...
    if (tournoi.Compo === 1) {
        ...
        StaffChannel.send({embed: {title: `Liste des participants au tournoi ${tournoi.NomduTournoi} :`, color: 'BLUE', fields: [{name: 'Participants :', value: inscrits, inline: true}, {name: 'Peudos de jeu :', value: pseudos, inline: true}]}, split: true})
    } else {
