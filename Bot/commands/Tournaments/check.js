...
module.exports.run = async (client, message) => {
    ...
    if (tournoi.NextRound[0] === "Finale") {
        ...
        await msgclassement.awaitReactions({filter: filterClassement, max: 1, time: 30000})
            .then(reaction => {
        ...
        try {
            for (let i=0; i<classement; i++) {
                ...
                await message.channel.awaitMessages({filter: filterMessages, max: 1, time: 30000})
                    .then(async coll => {
    ...
    mesg.react('✅')
    mesg.react('❌')
    await mesg.awaitReactions({filter: filterReaction, max: 1, time: 30000})
