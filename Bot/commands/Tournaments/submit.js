    ...
    await msg.awaitReactions({filter: filterReaction, max: 1, idle: 120000})
        .then(async coll => {
            ...
            case '✅':
                ...
                message.channel.send({embeds: [{color: 'GOLD', title: 'Vérification des résultats en cours...', footer: {text: 'Merci de patienter, le staff traite votre demande.'}}]})
                ...
                await StaffChannel.send({embeds: [{title: 'Liste des équipes du lobby concerné :', color: 'PURPLE', fields: [{name: 'Membres :', value: desc, inline: true}, {name: 'Pseudos:', value: field, inline: true}]}]})
                await StaffChannel.send({embeds: [{color: 'GOLD', description: `Faites \`${settings.prefix}check <teamID_or_userID>\` pour confirmer un résultat de lobby pour une équipe. Pour désigner plusieurs équipes gagnantes, listez les ID en les espaçant d'un espace.\n> Si tous les fichiers n'ont pas été envoyés, la totalité des fichiers se trouve en message épinglé dans le lobby : <#${message.channel.id}>`}]})
                break
            }
        })
}
