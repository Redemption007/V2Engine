...
module.exports = async (client, tournoiid) => {
    ...
    await InfosChannel.send('@everyone', {embed: {title: 'Début des inscriptions !', description: `Vous pouvez dorénavant vous inscrire dans le salon <#${InscriptionsChannel.id}>.\n> Fin des inscriptions le ${EndInscriptionsDate.toLocaleTimeString('fr-FR', options)}`, color: 'GREEN'}})
    await InscriptionsChannel.send({embed: {title: 'Début des inscriptions !', description: `Vous pouvez dorénavant vous inscrire au tournoi ${tournoi.NomduTournoi}.\n> Fin des inscriptions le ${EndInscriptionsDate.toLocaleTimeString('fr-FR', options)}\nL'inscription à ce tournoi donne le rôle <@&${tournoi.RoleTournoi}>`, color: 'GREEN'}})

    client.clock((IChannel, t) => {
        IChannel.send({embed: {title: 'Les inscriptions sont terminées !', color: 'RED', footer: {text: `Il est dorénavant trop tard pour vous inscrire. Le tournoi commence dans ${ms(t.Date.valueOf()-Date.now())}`}}})
