...
module.exports = async (client, message) => {
    ...
    if (message.author.bot || message.guild.id === '776429325838319616' && client.user.id==='705476031162613790') return
    ...
    const deleteWithoutError = message => {
    ...
    if (!command) return message.reply({embeds: [{color: 'RED', title: 'Oups !', description: "La commande demandée est inexistante..."}]}).then( msg => setTimeout(() => {
        return deleteWithoutError(msg)
    }, 5000))
    ...
    try {
        ...
        if (command.help.Modo && !message.member.permissions.has('KICK_MEMBERS')||command.help.Admin && !message.member.permissions.has('ADMINISTRATOR')||command.help.Animation && !message.member.permissions.has('ADMINISTRATOR') && !message.member.roles.cache.has(settings.animationrole)) {
            ...
            setTimeout(() => {
                return deleteWithoutError(message)
            }, 5000)
